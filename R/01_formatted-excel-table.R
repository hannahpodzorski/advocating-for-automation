# Example xlsx table with {openxlsx}
...
# Merge Cells
mergeCells(wb, sheet, rows = 3:(dim(d_sum)[1] + 2), cols = 2)
...
addStyle(wb, sheet, d_cen_style, rows = 3:(dim(d_sum)[1] + 2), cols = c(2, 4:(dim(d_sum)[2] + 1)), gridExpand = T, stack = T)
