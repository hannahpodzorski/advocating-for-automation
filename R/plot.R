...
ts_plot <- ggplot(data = d) + # set data
  geom_point(aes(x = date, y = result, color =  gm_chemical_name, shape = detected), size = 3) + # Add Points
  geom_path(aes(x = date, y = gm_result, color =  gm_chemical_name)) + # Add Lines
  scale_shape_manual(values = c("Detect" = 19, "Non-Detect" = 21), guide_legend(order = 2)) + # Define shapes based on detected column 
  scale_color_manual(values = c("Chloride" = plot_col[1]),  # defined colors based on gm_chemical_name
...
ts_plot <- rvg::dml(ggobj = ts_plot)

pptx <-read_pptx() %>%
  add_slide() %>%
  ph_with(ts_plot, ph_location(width = 8.75, height = 6.9))
